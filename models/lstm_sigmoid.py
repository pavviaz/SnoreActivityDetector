...
class LSTMSigmoid(Module):

    def __init__(self, input_size, hidden_size,
                 num_layers=1, bias=True, batch_first=False,
                 dropout=0, bidirectional=False):
        super(LSTMSigmoid, self).__init__()
    ...
    def _apply(self, fn):
        ret = super(LSTMSigmoid, self)._apply(fn)
        self.flatten_parameters()
        return ret
    ...
    def __setstate__(self, d):
        super(LSTMSigmoid, self).__setstate__(d)
...
def Recurrent(inner, reverse=False):
    def forward(input, hidden, weight):
        ...
        for i in steps:
            hidden = inner(True if reverse else False, input[i], hidden, *weight)
            # hack to handle LSTM
...
def VariableRecurrent(batch_sizes, inner):
    def forward(input, hidden, weight):
        ...
        for batch_size in batch_sizes:
            ...
            if flat_hidden:
                hidden = (inner(step_input, hidden[0], False, *weight),)
            else:
                hidden = inner(step_input, hidden, False, *weight)
...
def LSTMCell(reverse, input, hidden ,w_ih, w_hh, b_ih=None, b_hh=None):
    """
